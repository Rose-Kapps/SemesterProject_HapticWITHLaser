...
// variables of the plane fitting
Vec3 centroid;
Vec3 normal;
Vec3 singularValues;
Vec3 u;
...
//	normal = V.col(2); // column corresponding to smallest variance
//    normal.normalize();
//}
//
...
void updateHapticDevice(void)
{
    ...
    while (simulationRunning)
    {
        ...
        // with these scale factors, ignore THG signal
        ...
        if (state == STATE_TELEOPERATION)
        {
            ...
            else
            {
                ...
                // First haptic state: The user can move freely to find interface points
                if (haptic_state == FIND_INTERFACE) {
                    ...
                    // during fast positioning, no force feedback
                    if (scaleFactor == 0.2) {
                        F_normal.x(0.0);
                        F_normal.y(0.0);
                        F_normal.z(0.0);
                    ...
                    // If enough interface points (3 points) --> planar approximation + next state
                    ...
                    // correction in Vec3 to use eigen functions
