...
// 
// PLANAR_EXPLORATION: (haptic state)
// The operator is constrained to move along the estimated plane.
//------------------------------------------------------------------------------
enum cState
{
    STATE_IDLE,
...
// voltage values from the THG sensor
...
// to store the interface points detected
...
//Added variables and functions (by Daniel DaSilva)
ofstream outFile;
...
public:
    // Constructor
    LowPassFilter(double alpha = 0.1);

    // Apply the low-pass filter to a new input value
...
// =======================================
// Plane fitting and geometry declarations
// =======================================

// Fit a plane to a set of points using SVD
// Outputs: centroid, unit normal, singular values
...
// Build two orthonormal basis vectors lying in the plane
void planeBasis(const Vec3& normal,
    Vec3& u,
    Vec3& v);

// Check if a point is far enough from a set of points
...
void updateHapticDevice(void)
{
    ...
    while (simulationRunning)
    {
        ...
        // THG_signal_normalization (max value depends on the scale factor) --> TO ADAPT
        ...
        // normalized THG value 
        double THGsignal = cClamp((voltageLevel - min_voltage) / (max_voltage - min_voltage), 0.0, 1.0);  //////////////////////////

        // with these scale factors, we don't want any THG based haptic feedback!
        ...
        if (state == STATE_TELEOPERATION)
        {
            ...
            else
            {
                ...
                if (haptic_state == FIND_INTERFACE) {
                    ...
                    if (voltageLevel > 0.7) { // TO ADAPT

                        Vec3 interface_point = { robotPos.x(),robotPos.y(),robotPos.z() };
                        
                        double minDistance = 0.00005; // TO ADAPT
                    ...
                    if (list_of_interface_points.size() >= 3) { 
                    ...
                    double correction_factor = 0.5; // TO ADAPT
                    ...
                    double d = signedDistanceToPlane(virtualRobotPos_Vec3, centroid, normal);
					Vec3 hapticVel_Vec3(hapticVel.x(), hapticVel.y(), hapticVel.z());
					double vel_normal = hapticVel_Vec3.dot(normal);
...
// =======================================
// Plane fitting and geometry implementations
// =======================================
...
void fitPlaneSVD(const std::vector<Vec3>& points,
    ...
    // Plane normal = eigenvector associated with smallest singular value
...
void planeBasis(const Vec3& normal,
    ...
    // Choose a vector not collinear with the normal
    if (std::abs(n.x()) < 0.9) {
        tmp = Vec3(1, 0, 0);
    }
    else {
